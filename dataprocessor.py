...
def wash_data():
    with open('HP19mer_conf_data.txt', 'rb') as f:
...
def creat_training_data(test_set_size=1000):
    ...
    with open('HP19mer_symmetry_sample_ids.txt', 'rb') as f:
    ...
    with open(filetest, 'wb') as ftest:
        pickle.dump(dict_test, ftest)
    with open(filetrain, 'wb') as ftrain:
...
if __name__ == '__main__':
    ...
    for (_, _, f) in walk(datapath):
        filenames.extend(f)
        # eg. filename = '._HHHHHHHHHHHHPHPPPHP.conf'
        # eg. raw_conf = '[(0, 0), (0, 1), (1, 1), (1, 2), (0, 2), (0, 3), (-1, 3), (-2, 3),...,...]'
        # 处理带"_"和不带"_"的两种不同文件名，只需要不带下划线的

    # 1. 从filenames中读取input x；并从.conf文件读取output
    # 同一根链的逆向链作data augmentation
    raw_confs = []
    raw_confs_reverse = []
    input_HPs = []
    input_HPs_reverse = []
    dict0 = {'H': 0, 'P': 1}
    print('filenames的个数为：', len(filenames))
    _ = 0
    ...
    # output_confs: 取值[-1,0,1], dtype= int , len= 13454*2 = 26908
    # input_HPs: 取值[0, 1], dtype= np.float64, len= 26908
    ...
    for i in range(len(input_HPs)):
        ...
        if i % 100 == 0:
            print('ith-sample',i)
            print(len(washed_inputHPs),len(washed_outputconfs))
    print("最后剩余样本共多少个", len(washed_inputHPs))


    # 3. 将得到的datasets dump进入文件系统
    keys = ['num_of_sample', 'chain_length', 'input_HPs', 'output_confs']
